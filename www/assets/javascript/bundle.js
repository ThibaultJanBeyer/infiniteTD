...
this.bounty={value:bounty,creep:createElement('span','animation__gainmoney','+'+bounty),money:createElement('span','animation__gainmoney animation__gainmoney--scoreboard','+'+bounty),score:createElement('span','animation__gainscore animation__gainscore--scoreboard','+'+bounty)};bountyContainer.appendChild(this.bounty.creep);scoreboard.money.holder.appendChild(this.bounty.money);scoreboard.score.holder.appendChild(this.bounty.score);this.ms=ms;this.hp=hp;this.fullHp=hp;this.lasts=[];this.tolerance=5;this.i=0;creepContainer.appendChild(this.e);// set current position
...
var levels={creeps:{hp:10,ms:0.1,bounty:5,amount:1}},kills=0;/**************//* next level *//**************/function nextLevel(){// remove leftovers
var i=holders.length;while(i--){holders[i].innerHTML='';}creepContainer.innerHTML='';allCreeps=[];// next level
setTimeout(function(){levels.creeps.hp+=10;levels.creeps.ms+=0.01;levels.creeps.bounty+=0.1;levels.creeps.amount+=1;// recycling
recycleAnimation(recyclings);var tempCreeps=[];for(var _i4=0,il=levels.creeps.amount;_i4<il;_i4++){var creep=new Creeps({ms:levels.creeps.ms,hp:levels.creeps.hp,lvl:p1.level,bounty:levels.creeps.bounty});tempCreeps.push(creep);}myInterval({cd:tempCreeps.length,dur:500,cb:function cb(_ref6){var countdown=_ref6.countdown;tempCreeps[countdown].setup();allCreeps.push(tempCreeps[countdown]);}});},1000);}// users
