'use strict';var _typeof=typeof Symbol==="function"&&typeof Symbol.iterator==="symbol"?function(obj){return typeof obj;}:function(obj){return obj&&typeof Symbol==="function"&&obj.constructor===Symbol?"symbol":typeof obj;};var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}window.addEventListener("orientationchange",function(){alert(screen.orientation);// e.g. portrait 
});/* Globals */// available elements
var d=document,b=d.body,g=d.getElementsByClassName('infiniteTD')[0];// values
...
setSizes();b.onresize=function(){setSizes();};}(function(){setTimeout(function(){init();},500);})();/****************//* Setup Fields *//****************//* Field */var Field=function(){function Field(pos){var _this=this;_classCallCheck(this,Field);this.e=createElement('button','board__field');this.locked=false;this.start=false;this.end=false;this.pos=pos;// field click
...
wX=g.offsetWidth||g.clientWidth;wY=g.offsetHeight||g.clientHeight;// if the layout is horizontal
...
scoreboard.elements.forEach(function(e){e.classList.remove('scoreboard__el--alt');});scoreboard.e.classList.remove('scoreboard--alt');// if the layout is vertical
// set the game a square based on the width
}else{board.style.width=wX+'px';board.style.height=wX+'px';// also have the scoreboard elements inline
scoreboard.elements.forEach(function(e){e.classList.add('scoreboard__el--alt');});scoreboard.e.classList.add('scoreboard--alt');}// we will have to recalculate the exact
...
this.towerContainerE=[];this.towerOptionE=[];this.towerPriceE=[];this.towerInfoE=[];var _loop=function _loop(il,_i4){var option=options[_i4];var towerContainer=createElement('div','selector__container '+option.name);_this3.towerContainerE.push(towerContainer);var towerOption=createElement('button','selector__element '+option.name+'-name',option.nameOg);_this3.towerOptionE.push(towerOption);var towerPrice=createElement('span','selector__info '+option.name+'-price',option.cost+'$');_this3.towerPriceE.push(towerPrice);var towerInfo=createElement('button','selector__info '+option.name+'-showinfo','?');_this3.towerInfoE.push(towerPrice);towerInfo.addEventListener('click',function(e){e.stopPropagation();if(option.name==='sell'){extraInfo.upgrade(option);}else{extraInfo.tower(option);}});towerOption.addEventListener('click',function(e){e.stopPropagation();_this3.build(option,_this3.selectedField);});towerOption.appendChild(towerPrice);appendChilds(towerContainer,[towerOption,towerInfo]);_this3.e.appendChild(towerContainer);};for(var _i4=0,il=options.length;_i4<il;_i4++){_loop(il,_i4);}board.appendChild(this.e);// tower range
...
var els=['cost'];if(_tower.level!==0){els.push('level');}if(_tower.dmg!==0){els.push('dmg');}if(_tower.cd!==0){els.push('cd');}this.cont.floatsContainer=d.createElement('div');for(var _i5=0,il=els.length;_i5<il;_i5++){var info=els[_i5],val=_tower[els[_i5]],extra='';if(els[_i5]==='cost'){extra=' $';}else if(els[_i5]==='dmg'){info='damage';}else if(els[_i5]==='cd'){info='cooldown';extra=' sec';val=_tower[els[_i5]]/1000;}this.cont[els[_i5]]=createElement('div','extra-info__'+els[_i5]);this.cont[els[_i5]].text=createElement('div','extra-info__'+els[_i5]+'-text',info+':');this.cont[els[_i5]].val=createElement('div','extra-info__'+els[_i5]+'-value',''+val+extra);appendChilds(this.cont[els[_i5]],[this.cont[els[_i5]].text,this.cont[els[_i5]].val]);this.cont.floatsContainer.appendChild(this.cont[els[_i5]]);}// inner-container
...
function appendChilds(to,els){for(var _i6=0,il=els.length;_i6<il;_i6++){to.appendChild(els[_i6]);}}// myLoop
...
'gretel__breadcrumb','gretel__breadcrumb--top-left','gretel__breadcrumb--top-right','gretel__breadcrumb--bottom-left','gretel__breadcrumb--bottom-right','gretel__breadcrumb--bottom-right','gretel__breadcrumb--horizontal-last-left','gretel__breadcrumb--horizontal-last-right','gretel__breadcrumb--horizontal-last-top','gretel__breadcrumb--horizontal-next-bottom','gretel__breadcrumb--horizontal-next-left','gretel__breadcrumb--horizontal-next-right','gretel__breadcrumb--horizontal-next-top','gretel__breadcrumb--horizontal-next-bottom','gretel__breadcrumb--vertical-last-left','gretel__breadcrumb--vertical-last-right','gretel__breadcrumb--vertical-last-top','gretel__breadcrumb--vertical-last-bottom','gretel__breadcrumb--vertical-next-left','gretel__breadcrumb--vertical-next-right','gretel__breadcrumb--vertical-next-top','gretel__breadcrumb--vertical-next-bottom'];for(var _i7=0,il=_fields.length;_i7<il;_i7++){for(var j=0,jl=clearClasses.length;j<jl;j++){if(_fields[_i7].e.className.indexOf(clearClasses[j])>-1){_fields[_i7].e.classList.remove(clearClasses[j]);}}// re-lock all towers to be sure
if(_fields[_i7].e.className.indexOf('tower')>-1){_fields[_i7].lock('tower');}// get blocked _fields
if(_fields[_i7].locked&&_fields[_i7].pos!==startField.pos&&_fields[_i7].pos!==endField.pos&&_fields[_i7].locked==='tower'){grid.setWalkableAt(_fields[_i7].fX,_fields[_i7].fY,false);}}}// this fill draw smooth corners to the path
function handlePath(path){var corner=void 0,globFields=fields;for(var _i8=0,il=path.length;_i8<il;_i8++){// path[i][0] // fX coordinate
// path[i][1] // fY coordinate
// check if it is a corner field
var _fields=getFields(_i8,path);var checks=checkFields(_fields);corner='';for(var j=0,jl=checks.length;j<jl;j++){if(checks[j][0]){corner=checks[j][1];break;}}for(var k=0,kl=globFields.length;k<kl;k++){// get the field with the corresponding coordinates
if(globFields[k].fX===path[_i8][0]&&globFields[k].fY===path[_i8][1]){// add the breadcrumbs class
...
for(var _i9=0,il=fields.length;_i9<il;_i9++){if(fields[_i9].tower){// euclidean distance: https://en.wikipedia.org/wiki/Euclidean_distance
if(euclidDistance(el.x,fields[_i9].x,el.y,fields[_i9].y)<=fields[_i9].tower.rng){fields[_i9].tower.shoot(fields[_i9],el);}}}}// move element
