...
setSizes();b.onresize=function(){setSizes();};w.addEventListener('orientationchange',function(){setSizes();});}document.addEventListener('deviceready',function(){AndroidFullScreen.immersiveMode();isDevice=true;init();},false);(function(){setTimeout(function(){if(!isDevice){init();}},5000);})();/****************//* Setup Fields *//****************//* Field */var Field=function(){function Field(pos){var _this=this;_classCallCheck(this,Field);this.e=createElement('button','board__field');this.locked=false;this.start=false;this.end=false;this.pos=pos;// field click
...
if(!buK.upgrading&&!generalPause&&isStarted){sb.togglePlay();}}}builderOpen=false;}}},{key:'buildTower',value:function buildTower(tower){this.e.className+=' tower '+tower.name;this.e.setAttribute('data-level',tower.level);if(this.e.classList.contains('gretel__breadcrumb')){this.e.classList.remove('gretel__breadcrumb');}this.tower=tower;this.lock('tower');if(tower.cd!==0){this.scan();}gretel();}},{key:'destroyTower',value:function destroyTower(){this.e.classList.remove('tower',this.tower.name);this.e.removeAttribute('data-level');this.unlock();clearInterval(this.scanInterval);this.tower=0;}// scan for creeps nearby tower
},{key:'scan',value:function scan(){var _this2=this;var cooldown=0,tcooldown=this.tower.cd/100;// scan if creeps are nearby
this.scanInterval=setInterval(function(){if(!isPaused){cooldown+=1;if(cooldown>=tcooldown){var attacked=0,t=_this2.tower,trange=t.rng,ttargets=t.targets;// get all creeps
for(var _i2=0,il=allCreeps.length;_i2<il;_i2++){// check if the creeps distance is within tower range with
// euclidean distance: https://en.wikipedia.org/wiki/Euclidean_distance
if(euclidDistance(allCreeps[_i2].x,_this2.x,allCreeps[_i2].y,_this2.y)<=trange){// then check how many targets the tower can focus
if(attacked<=ttargets){t.shoot(_this2,allCreeps[_i2]);attacked++;cooldown=0;}}}}}},100);}}]);return Field;}();function handleFieldClick(el,e){var bu=builders,sb=scoreboard;el.position();if(!el.locked){el.openBuilder(bu.towers);}else if(el.locked==='tower'){checkWhichTowerToOpen(el,bu,sb);}else if((el.start||el.end)&&!builderOpen){audio.play('do_not_block');}else{// traverse all possible builders
...
var board=void 0,fields=[],startField=void 0,endField=void 0,topFields=[],rightFields=[],bottomFields=[],leftFields=[],// values
...
for(var _i3=0,il=boardSize;_i3<il;_i3++){// create each field
fields[_i3]=new Field(_i3);// give fields x & y coordinates
...
// [3,0][3,1][3,2]
//
fields[_i3].fX=tempX;fields[_i3].fY=tempY;// give x +1 until last
// if last x = 0 and y + 1
if(tempX<boardRowSize-1){tempX++;}else{tempX=0;tempY++;}// lock start & endfield
if(_i3===startFieldP){fields[_i3].lock('start');startField=fields[_i3];}else if(_i3===endFieldP-1){fields[_i3].lock('end');endField=fields[_i3];}// append field to board
board.appendChild(fields[_i3].e);}// append to game
...
g.addEventListener('click',function(e){e.stopPropagation();globalClick(e);});// pause / unpause game with spacebar
g.addEventListener('keyup',function(e){globalKeyboard(e);});}function globalClick(e){var bu=builders,sb=scoreboard;for(var key in bu){if(bu.hasOwnProperty(key)&&bu[key].selectedField){var buK=bu[key];// hide it
buK.hide();// unpause if the builder was not an upgrade builder
...
wX=b.offsetWidth||b.clientWidth;wY=b.offsetHeight||b.clientHeight;// if the layout is horizontal
...
scoreboard.elements.forEach(function(e){e.classList.remove('scoreboard__el--alt');});scoreboard.e.classList.remove('scoreboard--alt');board.classList.remove('board--alt');var _i4=fields.length;while(_i4--){fields[_i4].e.style.width=board.clientWidth/10+'px';fields[_i4].e.style.height=board.clientHeight/10+'px';}// if the layout is vertical
...
scoreboard.elements.forEach(function(e){e.classList.add('scoreboard__el--alt');});scoreboard.e.classList.add('scoreboard--alt');board.classList.add('board--alt');var _i5=fields.length;while(_i5--){fields[_i5].e.style.width=board.clientWidth/10+'px';fields[_i5].e.style.height=board.clientHeight/10+'px';}}// we will have to recalculate the exact
...
var j=allTowers.length;while(j--){allTowers[j].update();}}// Make board accessible via Keyboard
...
keyboardBasicMovement(key,cases);keyboardEscape(key);}function keyboardBasicMovement(key,cases){var num=[[39,'ArrowRight'],[37,'ArrowLeft'],[38,'ArrowUp'],[40,'ArrowDown']];var i=num.length;while(i--){if((key===num[i][0]||key===num[i][1])&&!cases[i].edge){cases[i].field.e.focus();}}}function keyboardEscape(k){var bu=builders,sb=scoreboard;if(k===27||k==='Escape'){for(var key in bu){if(bu.hasOwnProperty(key)){bu[key].hide();}}sb.play.focus();}}/* Builder */var Builder=function(){function Builder(options){var _this3=this;_classCallCheck(this,Builder);this.selectedField=false;this.e=createElement('div','selector');this.e.addEventListener('click',function(e){e.stopPropagation();_this3.hide();});// add tower options
this.towerContainerE=[];this.towerOptionE=[];this.towerPriceE=[];this.towerInfoE=[];var _loop=function _loop(il,_i6){var option=options[_i6];var towerContainer=createElement('div','selector__container '+option.name);_this3.towerContainerE.push(towerContainer);var towerOption=createElement('button','selector__element '+option.name+'-name',option.nameOg);_this3.towerOptionE.push(towerOption);var towerPrice=createElement('span','selector__info '+option.name+'-price',option.cost+'$');_this3.towerPriceE.push(towerPrice);var towerInfo=createElement('button','selector__info '+option.name+'-showinfo','?');_this3.towerInfoE.push(towerPrice);towerInfo.addEventListener('click',function(e){e.stopPropagation();if(option.name==='sell'){extraInfo.upgrade(option);}else{extraInfo.tower(option);}});towerOption.addEventListener('click',function(e){e.stopPropagation();_this3.build(option,_this3.selectedField);});towerOption.appendChild(towerPrice);appendChilds(towerContainer,[towerOption,towerInfo]);_this3.e.appendChild(towerContainer);};for(var _i6=0,il=options.length;_i6<il;_i6++){_loop(il,_i6);}board.appendChild(this.e);// tower range
this.range=createElement('div','tower__range');this.range.addEventListener('click',function(e){e.stopPropagation();_this3.hide();});board.appendChild(this.range);// record keyboard input
this.e.addEventListener('keyup',function(e){userBuilderWithKey(_this3,e);});}_createClass(Builder,[{key:'draw',value:function draw(field,upgrade){this.selectedField=field;this.selectedField.builder=this;field.builder=this;if(!upgrade){this.upgrading=false;field.lock('tower');// pause
...
if(!gretel()){if(!this.upgrading){field.unlock();}audio.play('do_not_block');this.hide();}else{if(!this.upgrading){field.unlock();}this.w=field.w*2+'px';this.h=field.w*2+'px';this.e.className='selector selector--show';this.e.style.width=this.w;this.e.style.height=this.h;this.e.style.left=field.x-field.w/2+'px';this.e.style.top=field.y-field.w/2+'px';}}},{key:'hide',value:function hide(){var general=arguments.length<=0||arguments[0]===undefined?false:arguments[0];if(this.selectedField){this.selectedField.builder=false;this.selectedField=false;}// unpause & remove the show classes from builders
...
var allCreeps=[];/* Creeps */var Creeps=function(){function Creeps(_ref2){var ms=_ref2.ms;var hp=_ref2.hp;var lvl=_ref2.lvl;var bounty=_ref2.bounty;_classCallCheck(this,Creeps);this.e=createElement('div','c c__l'+lvl);this.ms=ms;this.hp=hp;this.fullHp=hp;this.bounty=bounty;this.lasts=[];this.tolerance=5;// visually represent hitpoints
// this.hp / this.fullHp = 0.5 at 50% hp
// * 10 to get a full number value so that .ceil rounds to a full number properly
this.e.setAttribute('data-hp',Math.ceil(this.hp/this.fullHp*10));}_createClass(Creeps,[{key:'create',value:function create(){board.appendChild(this.e);this.e.style.left=startField.x+'px';this.e.style.top=startField.y+'px';nextLocation(this);}},{key:'damage',value:function damage(dmg){this.hp-=dmg;this.e.setAttribute('data-hp',Math.ceil(this.hp/this.fullHp*10));if(this.hp<=0){this.remove(true,p1);}}},{key:'remove',value:function remove(killed,player){if(!this.dead){// dead
this.dead=true;if(killed){player.unitKill(this);}// remove creep
// from board
board.removeChild(this.e);// from allCreeps array
allCreeps.splice(allCreeps.indexOf(this),1);kills++;if(kills>=levels[p1.level].amount){kills=0;p1.levelUp();}}}}]);return Creeps;}();// the variable gretelFields contains all fields set by gretels path coordinates
function nextLocation(creep){var i=arguments.length<=1||arguments[1]===undefined?0:arguments[1];var gf=gretelFields;// check if creep is not dead
if(!creep.dead){var length=gretelFields.length-1;if(i++<length){// set current position
creep.x=parseInt(creep.e.style.left);creep.y=parseInt(creep.e.style.top);// move to next position
moveCreep(creep,gf[i],function(el){nextLocation(el,i);});}else{var afterEnd={x:creep.x+endField.w/3,y:creep.y};moveCreep(creep,afterEnd,function(){p1.updateLives(-1);creep.remove();});}}}// move creep
function moveCreep(el,next,cb){var increment=void 0;if(!isPaused){// calculate the distance
// (x:10,y:20)[cur] -dist-> [next](x:20,y:20)
// next.x(20) - cur.x(10) = +10 dist
// next.y(20) - cur.y(20) = 0 dist
el.dist={x:next.x-el.x,y:next.y-el.y};increment=calculateIncrement(el,next);el.x+=increment.x;el.dist.x-=increment.x;el.y+=increment.y;el.dist.y-=increment.y;// update creep
el.e.style.left=el.x+'px';el.e.style.top=el.y+'px';}if(typeof increment==='undefined'||increment==null||increment.steps>=1){setTimeout(function(){return moveCreep(el,next,cb);},10);}else{return cb(el,next,cb);}}// see https://github.com/edenspiekermann/a11y-dialog/ for more information
...
var els=['cost'];if(_tower.level!==0){els.push('level');}if(_tower.dmg!==0){els.push('dmg');}if(_tower.cd!==0){els.push('cd');}this.cont.floatsContainer=d.createElement('div');for(var _i7=0,il=els.length;_i7<il;_i7++){var info=els[_i7],val=_tower[els[_i7]],extra='';if(els[_i7]==='cost'){extra=' $';}else if(els[_i7]==='dmg'){info='damage';}else if(els[_i7]==='cd'){info='cooldown';extra=' sec';val=_tower[els[_i7]]/1000;}this.cont[els[_i7]]=createElement('div','extra-info__'+els[_i7]);this.cont[els[_i7]].text=createElement('div','extra-info__'+els[_i7]+'-text',info+':');this.cont[els[_i7]].val=createElement('div','extra-info__'+els[_i7]+'-value',''+val+extra);appendChilds(this.cont[els[_i7]],[this.cont[els[_i7]].text,this.cont[els[_i7]].val]);this.cont.floatsContainer.appendChild(this.cont[els[_i7]]);}// inner-container
...
this.dialog.show();}}]);return ExtraInfo;}();function setupExtraInfo(){extraInfo=new ExtraInfo();}function animateScore(_ref3){var className=_ref3.className;var value=_ref3.value;var pos1=_ref3.pos1;var pos2=_ref3.pos2;if(pos1){(function(){var eBoard=createElement('div',className,value);eBoard.style.left=pos1.x+'px';eBoard.style.top=pos1.y+'px';board.appendChild(eBoard);setTimeout(function(){board.removeChild(eBoard);},1000);})();}if(pos2){(function(){var eScore=createElement('div',className+' '+className+'--scoreboard',value);pos2.appendChild(eScore);setTimeout(function(){pos2.removeChild(eScore);},1000);})();}}// this function calculates the x and y increments
...
function calculateIncrement(el,next){var increment={};if(el.follow){el.dist={x:next.x-el.x,y:next.y-el.y};}var x=Math.abs(el.dist.x);var y=Math.abs(el.dist.y);if(x>y){// 1.
increment.x=el.ms;increment.steps=x/el.ms;// 2.
increment.y=y/increment.steps;// 3.
}else{// 1.
increment.y=el.ms;increment.steps=y/el.ms;// 2.
increment.x=x/increment.steps;// 3.
}// 4.
if(el.dist.x<0){increment.x*=-1;}if(el.dist.y<0){increment.y*=-1;}return increment;}/******************************//* Functions for an easy life *//******************************/// capitalize first letter
...
function appendChilds(to,els){for(var _i8=0,il=els.length;_i8<il;_i8++){to.appendChild(els[_i8]);}}// myLoop
// pass number of iterations and dur in ms and counter
function myLoop(_ref4){var cd=_ref4.cd;var dur=_ref4.dur;var cu=_ref4.cu;var cb=_ref4.cb;// passes usefull stuff to callback and augmet the count up by 1
...
function myInterval(_ref5){var cd=_ref5.cd;var dur=_ref5.dur;var cb=_ref5.cb;var countdown=cd;var loop=setInterval(interval,dur);function interval(){if(!isPaused){if(--countdown>=0){cb({cd:cd,dur:dur});}else{clearInterval(loop);}}}}// euclidean distance: https://en.wikipedia.org/wiki/Euclidean_distance 
function euclidDistance(x1,x2,y1,y2){return Math.sqrt(Math.pow(x1-x2,2)+Math.pow(y1-y2,2));}// tried to write that AI pathfinding myself
...
'gretel__breadcrumb','gretel__breadcrumb--top-left','gretel__breadcrumb--top-right','gretel__breadcrumb--bottom-left','gretel__breadcrumb--bottom-right','gretel__breadcrumb--bottom-right','gretel__breadcrumb--horizontal-last-left','gretel__breadcrumb--horizontal-last-right','gretel__breadcrumb--horizontal-last-top','gretel__breadcrumb--horizontal-next-bottom','gretel__breadcrumb--horizontal-next-left','gretel__breadcrumb--horizontal-next-right','gretel__breadcrumb--horizontal-next-top','gretel__breadcrumb--horizontal-next-bottom','gretel__breadcrumb--vertical-last-left','gretel__breadcrumb--vertical-last-right','gretel__breadcrumb--vertical-last-top','gretel__breadcrumb--vertical-last-bottom','gretel__breadcrumb--vertical-next-left','gretel__breadcrumb--vertical-next-right','gretel__breadcrumb--vertical-next-top','gretel__breadcrumb--vertical-next-bottom'];for(var _i9=0,il=_fields.length;_i9<il;_i9++){for(var j=0,jl=clearClasses.length;j<jl;j++){if(_fields[_i9].e.className.indexOf(clearClasses[j])>-1){_fields[_i9].e.classList.remove(clearClasses[j]);}}// re-lock all towers to be sure
if(_fields[_i9].e.className.indexOf('tower')>-1){_fields[_i9].lock('tower');}// get blocked _fields
if(_fields[_i9].locked&&_fields[_i9].pos!==startField.pos&&_fields[_i9].pos!==endField.pos&&_fields[_i9].locked==='tower'){grid.setWalkableAt(_fields[_i9].fX,_fields[_i9].fY,false);}}}// this fill draw smooth corners to the path
function handlePath(path){var corner=void 0,globFields=fields;for(var _i10=0,il=path.length;_i10<il;_i10++){// path[i][0] // fX coordinate
// path[i][1] // fY coordinate
// check if it is a corner field
var _fields=getFields(_i10,path);var checks=checkFields(_fields);corner='';for(var j=0,jl=checks.length;j<jl;j++){if(checks[j][0]){corner=checks[j][1];break;}}for(var k=0,kl=globFields.length;k<kl;k++){// get the field with the corresponding coordinates
if(globFields[k].fX===path[_i10][0]&&globFields[k].fY===path[_i10][1]){// add the breadcrumbs class
globFields[k].e.className+=' gretel__breadcrumb '+corner;gretelFields.push(globFields[k]);}}}}// grabs the current, the next and previous field on the path for further calculations
...
gretel();}// gui
var scoreboard=void 0,sounds={'resources':[['assets/sounds/sprite.ogg','audio/mp3'],['assets/sounds/sprite.mp3','audio/ogg']],'spritemap':{'cannot_build_here':{'start':0,'end':1.3,'loop':false},'do_not_block':{'start':3,'end':4,'loop':false},'finish_building_process_first':{'start':5,'end':7.1,'loop':false},'need_more_money':{'start':9,'end':10.2,'loop':false},'winner_winner_chicken_dinner':{'start':12,'end':13.5,'loop':false},'you_lost_try_again':{'start':15,'end':17.3,'loop':false}}},audio=void 0,soundOff=false;/* Scoreboard */var Scoreboard=function(){function Scoreboard(){var _this4=this;_classCallCheck(this,Scoreboard);this.e=createElement('div','scoreboard');this.m=createElement('p','scoreboard__el scoreboard__el-message','.');this.money=createSVG({svgName:'money',extraElement:'p',svg:SVGmoney});this.player=createSVG({svgName:'player',extraElement:'input',svg:SVGplayer});this.player.input.setAttribute('aria-label','Player name: ');this.level=createSVG({svgName:'level',extraElement:'p',svg:SVGlevel});this.score=createSVG({svgName:'score',extraElement:'p',svg:SVGscore});this.lives=createSVG({svgName:'lives',extraElement:'p',svg:SVGlives});this.controls=createElement('div','scoreboard__el-controls');this.play=createElement('button','scoreboard__el scoreboard__el-pause','play');this.audioOff=createSVG({container:'button',svgName:'audio',svg:SVGaudio.off});this.audioOn=createSVG({container:'button',svgName:'audio',svg:SVGaudio.on});this.audioOn.container.style.display='none';appendChilds(this.controls,[this.play,this.audioOff.container,this.audioOn.container]);// Global Play & Pause
 ...
 */// Create Svg
function createSVG(_ref6){var _ref6$containerClass=_ref6.containerClass;var containerClass=_ref6$containerClass===undefined?'scoreboard__el':_ref6$containerClass;var _ref6$container=_ref6.container;var container=_ref6$container===undefined?'div':_ref6$container;var svg=_ref6.svg;var svgName=_ref6.svgName;var _ref6$extraElement=_ref6.extraElement;var extraElement=_ref6$extraElement===undefined?false:_ref6$extraElement;var el={};el.container=createElement(container,containerClass+' '+containerClass+'-'+svgName,svg);if(extraElement){el[extraElement]=createElement(extraElement,containerClass+'-'+extraElement);el.container.appendChild(el[extraElement]);}return el;}// SVGs
...
var levels={},kills=0;// levels 1-10
var i=10;while(i--){if(i===1){levels[i]={creeps:{hp:100*i,ms:10,bounty:5},amount:1};}else{levels[i]={creeps:{hp:10*i,ms:1,bounty:5},amount:2*i};}}/**************//* next level *//**************/function nextLevel(){setTimeout(function(){myInterval({cd:levels[p1.level].amount,dur:500,cb:function cb(){var creep=new Creeps({ms:levels[p1.level].creeps.ms,hp:levels[p1.level].creeps.hp,lvl:p1.level,bounty:levels[p1.level].creeps.bounty});creep.create();allCreeps.push(creep);}});},1000);}// users
var p1=void 0;/* Player */var Player=function(){function Player(_ref7){var name=_ref7.name;var money=_ref7.money;var level=_ref7.level;var score=_ref7.score;var lives=_ref7.lives;_classCallCheck(this,Player);this.name=name;this.money=money;this.level=level;this.score=score;this.lives=lives;}_createClass(Player,[{key:'unitKill',value:function unitKill(unit){this.money+=unit.bounty;this.score+=unit.bounty;scoreboard.update(this);animateScore({className:'animation__gainmoney',value:'+'+unit.bounty+' $',pos1:unit,pos2:scoreboard.money.container});animateScore({className:'animation__gainpoints',value:'+'+unit.bounty,pos2:scoreboard.score.container});}},{key:'levelUp',value:function levelUp(){var _this7=this;if(!lostGame){this.level+=1;animateScore({className:'animation__levelup',value:'+1',pos1:startField,pos2:scoreboard.level.container});scoreboard.update(this);if(!levels[this.level]){audio.play('winner_winner_chicken_dinner');}else{nextLevel();}setTimeout(function(){if(!lostGame){if(_this7.level%2===0){_this7.updateLives(1);}}},1000);}}},{key:'updateLives',value:function updateLives(amount){// lose life
this.lives+=amount;if(amount>=0){animateScore({className:'animation__gainlives',value:'+'+amount,pos2:scoreboard.lives.container});}else{animateScore({className:'animation__loselives',value:amount,pos1:endField,pos2:scoreboard.lives.container});}scoreboard.update(this);// check if lost
if(this.lives<=0){lost();}}},{key:'updateMoney',value:function updateMoney(amount,place){// update wallet
this.money+=amount;if(amount>=0){animateScore({className:'animation__gainmoney',value:'+'+amount+' $',pos1:place,pos2:scoreboard.money.container});}else{animateScore({className:'animation__losemoney',value:amount+' $',pos1:place,pos2:scoreboard.money.container});}scoreboard.update(this);}}]);return Player;}();function setupPlayer(){p1=new Player({name:'Player 1',money:100,level:0,score:0,lives:10});scoreboard.update(p1);}function lost(){var i=allCreeps.length;while(i--){allCreeps[i].remove();}var allTowers=board.querySelectorAll('.t');var j=allTowers.length;while(j--){allTowers[j].className='board__field';}lostGame=true;isPaused=true;audio.play('you_lost_try_again');}// @TODO: remove cheats
setInterval(function(){if(p1.name==='t'){p1.updateLives(9999);p1.updateMoney(9999);}},5000);// Towers
var tBasic=void 0,sell=void 0,rock=void 0,builders={},allTowers=[];/* projectile */var Projectile=function(){function Projectile(field,creep){_classCallCheck(this,Projectile);this.field=field;this.ms=field.tower.pms;this.dmg=field.tower.dmg;this.x=field.x;this.y=field.y;this.follow=field.tower.follow;this.e=createElement('div','projectile projectile__'+field.tower.name);this.e.style.left=this.x+'px';this.e.style.top=this.y+'px';board.appendChild(this.e);moveProjectile(this,creep);}_createClass(Projectile,[{key:'remove',value:function remove(){// @TODO: add explosion
// remove element
// dead
this.dead=true;// from board
board.removeChild(this.e);}}]);return Projectile;}();/* Tower */var Tower=function(){function Tower(_ref8){var name=_ref8.name;var cost=_ref8.cost;var _ref8$dmg=_ref8.dmg;var dmg=_ref8$dmg===undefined?0:_ref8$dmg;var _ref8$cd=_ref8.cd;var cd=_ref8$cd===undefined?0:_ref8$cd;var rng=_ref8.rng;var pms=_ref8.pms;var _ref8$targets=_ref8.targets;var targets=_ref8$targets===undefined?0:_ref8$targets;var _ref8$follow=_ref8.follow;var follow=_ref8$follow===undefined?true:_ref8$follow;var _ref8$level=_ref8.level;var level=_ref8$level===undefined?1:_ref8$level;var description=_ref8.description;_classCallCheck(this,Tower);this.nameOg=name;this.name='tower__'+name;this.cost=cost;this.dmg=dmg;this.cd=cd;this.rngVal=rng;this.rng=fields[0].w+rng*fields[0].w;this.pms=pms;this.targets=targets;this.follow=follow;this.level=level;this.description=description;}_createClass(Tower,[{key:'shoot',value:function shoot(field,creep){if(this.dmg!=null){var porjectile=new Projectile(field,creep);}}},{key:'update',value:function update(){this.rng=fields[0].w+this.rngVal*fields[0].w;}}]);return Tower;}();function setupTowers(){tBasic=new Tower({name:'basic',cost:50,dmg:50,pms:20,cd:1000,rng:0.6,description:'This tower has a high attack speed with a basic damage and range. Upgrades drastically improve its range. Moreover, with special researches, this tower will be key to your success.'});allTowers.push(tBasic);rock=new Tower({name:'rock',cost:6,level:0,description:'Simple & cheap rock. Usefull to block a path.'});sell=new Tower({name:'sell',cost:'+½',level:0,description:'You can sell any tower and get back the half of its cost.'});builders.towers=new Builder([tBasic,rock]);builders.basic=new Builder([sell]);builders.rock=new Builder([sell]);}function towersInRange(el){// check if any Tower is in range
for(var _i11=0,il=fields.length;_i11<il;_i11++){if(fields[_i11].tower){// euclidean distance: https://en.wikipedia.org/wiki/Euclidean_distance
if(euclidDistance(el.x,fields[_i11].x,el.y,fields[_i11].y)<=fields[_i11].tower.rng){fields[_i11].tower.shoot(fields[_i11],el);}}}}// move element
function moveProjectile(el,creep){// calculate the distance
// (x:10,y:20)[cur] -dist-> [next](x:20,y:20)
// next.x(20) - cur.x(10) = +10 dist
// next.y(20) - cur.y(20) = 0 dist
el.dist={x:creep.x-el.x,y:creep.y-el.y};el.angleDeg=Math.atan2(creep.y-el.y,creep.x-el.x)*180/Math.PI;el.e.style.transform='translate(100%, 500%) rotate('+el.angleDeg+'deg)';var loop=setInterval(interval,20);function interval(){if(!isPaused){var increment=calculateIncrement(el,creep);if(el.follow){el.angleDeg=Math.atan2(creep.y-el.y,creep.x-el.x)*180/Math.PI;el.e.style.transform='translate(100%, 500%) rotate('+el.angleDeg+'deg)';}el.x+=increment.x;el.dist.x-=increment.x;el.y+=increment.y;el.dist.y-=increment.y;if(increment.steps<1.5){creep.damage(el.dmg);el.remove();clearInterval(loop);}else{el.e.style.left=el.x+'px';el.e.style.top=el.y+'px';}}}}/*
...
if(ex.number===-0x7FF5EC54){classListPropDesc.enumerable=false;objCtr.defineProperty(elemCtrProto,classListProp,classListPropDesc);}}}else if(objCtr[protoProp].__defineGetter__){elemCtrProto.__defineGetter__(classListProp,classListGetter);}})(self);}else{// There is full or partial native classList support, so just check if we need
