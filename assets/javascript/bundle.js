...
setSizes();b.onresize=function(){setSizes();};w.addEventListener('orientationchange',function(){setSizes();});}document.addEventListener('deviceready',function(){AndroidFullScreen.immersiveMode();isDevice=true;init();},false);(function(){setTimeout(function(){if(!isDevice){init();}},2000);})();/****************//* Setup Fields *//****************//* Field */var Field=function(){function Field(pos){var _this=this;_classCallCheck(this,Field);this.e=createElement('button','board__field');this.locked=false;this.start=false;this.end=false;this.pos=pos;// field click
...
if(!buK.upgrading&&!generalPause&&isStarted){sb.togglePlay();}}}builderOpen=false;}}},{key:'buildTower',value:function buildTower(tower){this.e.className+=' tower '+tower.name;this.e.setAttribute('data-level',tower.level);if(this.e.className.indexOf('gretel__breadcrumb')>-1){removeClass(this.e,'gretel__breadcrumb');}this.tower=tower;this.lock('tower');if(tower.cd!==0){this.scan();}gretel();}},{key:'destroyTower',value:function destroyTower(){removeClass(this.e,'tower');removeClass(this.e,this.tower.name);this.e.removeAttribute('data-level');this.unlock();clearInterval(this.scanInterval);this.tower=0;}// scan for creeps nearby tower
...
var board=void 0,creepContainer=void 0,projectileContainer=void 0,fields=[],startField=void 0,endField=void 0,topFields=[],rightFields=[],bottomFields=[],leftFields=[],// values
...
board=createElement('div','board');// create creep & projectile container
creepContainer=d.createElement('div');projectileContainer=d.createElement('div');appendChilds(board,[creepContainer,projectileContainer]);// fields
...
scoreboard.elements.forEach(function(e){removeClass(e,'scoreboard__el--alt');});removeClass(scoreboard.e,'scoreboard--alt');removeClass(board,'board--alt');var _i4=fields.length;while(_i4--){fields[_i4].e.style.width=board.clientWidth/10+'px';fields[_i4].e.style.height=board.clientHeight/10+'px';}// if the layout is vertical
...
scoreboard.elements.forEach(function(e){addClass(e,'scoreboard__el--alt');});addClass(scoreboard.e,'scoreboard--alt');addClass(board,'board--alt');var _i5=fields.length;while(_i5--){fields[_i5].e.style.width=board.clientWidth/10+'px';fields[_i5].e.style.height=board.clientHeight/10+'px';}}// we will have to recalculate the exact
...
var pos=[field.x-field.w/2,field.y-field.w/2];console.log(parseInt(this.w));if(parseInt(this.w)<=90){pos=[field.x-35,field.y-35];console.log('inside');}console.log(pos);this.e.style.left=pos[0]+'px';this.e.style.top=pos[1]+'px';}}},{key:'hide',value:function hide(){var general=arguments.length<=0||arguments[0]===undefined?false:arguments[0];if(this.selectedField){this.selectedField.builder=false;this.selectedField=false;}// unpause & remove the show classes from builders
...
function myLoop(_ref4){var cd=_ref4.cd;var dur=_ref4.dur;var _ref4$cu=_ref4.cu;var cu=_ref4$cu===undefined?0:_ref4$cu;var cb=_ref4.cb;// passes usefull stuff to callback and augmet the count up by 1
...
function myInterval(_ref5){var cd=_ref5.cd;var dur=_ref5.dur;var cb=_ref5.cb;var countdown=cd;var loop=setInterval(interval,dur);function interval(){if(!isPaused){if(--countdown>=0){cb({countdown:countdown,dur:dur});}else{clearInterval(loop);}}}}// euclidean distance: https://en.wikipedia.org/wiki/Euclidean_distance 
...
var tBasic=void 0,sell=void 0,rock=void 0,builders={},allTowers=[];/* projectile */var Projectile=function(){function Projectile(field,creep){_classCallCheck(this,Projectile);this.field=field;this.ms=field.tower.pms;this.dmg=field.tower.dmg;this.x=field.x;this.y=field.y;this.follow=field.tower.follow;this.e=createElement('div','projectile projectile__'+field.tower.name);this.e.style.left=this.x+'px';this.e.style.top=this.y+'px';projectileContainer.appendChild(this.e);moveProjectile(this,creep);}_createClass(Projectile,[{key:'remove',value:function remove(){// @TODO: add explosion
// remove element
// dead
this.dead=true;// from board
projectileContainer.removeChild(this.e);}}]);return Projectile;}();/* Tower */var Tower=function(){function Tower(_ref9){var name=_ref9.name;var cost=_ref9.cost;var _ref9$dmg=_ref9.dmg;var dmg=_ref9$dmg===undefined?0:_ref9$dmg;var _ref9$cd=_ref9.cd;var cd=_ref9$cd===undefined?0:_ref9$cd;var rng=_ref9.rng;var pms=_ref9.pms;var _ref9$targets=_ref9.targets;var targets=_ref9$targets===undefined?0:_ref9$targets;var _ref9$follow=_ref9.follow;var follow=_ref9$follow===undefined?true:_ref9$follow;var _ref9$level=_ref9.level;var level=_ref9$level===undefined?1:_ref9$level;var description=_ref9.description;_classCallCheck(this,Tower);this.nameOg=name;this.name='tower__'+name;this.cost=cost;this.dmg=dmg;this.cd=cd;this.rngVal=rng;this.rng=fields[0].w+rng*fields[0].w;this.pms=pms;this.targets=targets;this.follow=follow;this.level=level;this.description=description;}_createClass(Tower,[{key:'shoot',value:function shoot(field,creep){if(this.dmg!=null){var porjectile=new Projectile(field,creep);}}},{key:'update',value:function update(){this.rng=fields[0].w+this.rngVal*fields[0].w;}}]);return Tower;}();function setupTowers(){tBasic=new Tower({name:'basic',cost:50,dmg:50,pms:20,cd:1000,rng:0.6,description:'This tower has a high attack speed with a basic damage and range. Upgrades drastically improve its range. Moreover, with special researches, this tower will be key to your success.'});allTowers.push(tBasic);rock=new Tower({name:'rock',cost:6,level:0,description:'Simple & cheap rock. Usefull to block a path.'});sell=new Tower({name:'sell',cost:'+½',level:0,description:'You can sell any tower and get back the half of its cost.'});builders.towers=new Builder([tBasic,rock]);builders.basic=new Builder([sell]);builders.rock=new Builder([sell]);}function towersInRange(el){// check if any Tower is in range of creep
for(var _i12=0,il=fields.length;_i12<il;_i12++){if(fields[_i12].tower){// euclidean distance: https://en.wikipedia.org/wiki/Euclidean_distance
// and the ennemy is not invulnerable
if(euclidDistance(el.x,fields[_i12].x,el.y,fields[_i12].y)<=fields[_i12].tower.rng&&!el.invulnerable){fields[_i12].tower.shoot(fields[_i12],el);}}}}// move element
function moveProjectile(el,creep){// calculate the distance
// (x:10,y:20)[cur] -dist-> [next](x:20,y:20)
// next.x(20) - cur.x(10) = +10 dist
// next.y(20) - cur.y(20) = 0 dist
el.dist={x:creep.x-el.x,y:creep.y-el.y};var loop=setInterval(interval,20);function interval(){if(!isPaused){var increment=calculateIncrement(el,creep);el.x+=increment.x;el.dist.x-=increment.x;el.y+=increment.y;el.dist.y-=increment.y;if(increment.steps<1.5){creep.damage(el.dmg);el.remove();clearInterval(loop);}else{el.e.style.left=el.x+'px';el.e.style.top=el.y+'px';}}}}/**
